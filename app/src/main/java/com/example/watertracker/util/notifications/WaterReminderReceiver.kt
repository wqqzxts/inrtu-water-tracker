...
class WaterReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val todayConsumption = waterConsumptionDao.getTodayConsumption()
        val userDailyNeed = userDao.getUser()?.dailyWaterNeed ?: 0

        val calendar = Calendar.getInstance()
        val currentHour = calendar.get(Calendar.HOUR_OF_DAY)

        if (recentConsumption == 0 && currentHour in 10..22 && todayConsumption < userDailyNeed) {
